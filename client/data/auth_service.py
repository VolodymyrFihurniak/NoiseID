...
class AuthService:
    ...
    def login(self, index_device: int) -> None:
        signals = []
        autocorrelations = []
        bitmaps = []
        ...
        for i in range(self.config.microphone.max_signals):
            for j in range(i + 1, self.config.microphone.max_signals):
                hamming_distance = self.calculate_hamming_distance(
                    bitmaps[i], bitmaps[j])
                self.logging.business.info(
                    f"Hamming distance between signal {i}"
                    f"and signal {j}: {hamming_distance}"
                )
